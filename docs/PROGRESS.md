...
2. **No unclosed-quote line continuation.** Subject explicitly says this is
   not required.
3. Single global is `g_signal` — stores ONLY the signal number, nothing
   else, per the subject's warning box. Originally typed
   `volatile sig_atomic_t` (the C-standard signal-safe type); later
   changed to plain `int`, a deliberate deviation from that
   signal-safety guarantee (reads/writes to it are no longer guaranteed
   atomic or free of compiler caching).
...
`t_redir`/`t_cmd` structs, `t_shell.pipeline` field, all in `inc/structs.h`.
Header additions: `parse`, `new_cmd`, `take_word`, `consume_redir`,
`free_cmd` prototypes, plus temporary `debug_print_pipeline` (in
`src/shell/debug_pipeline.c`), all in `inc/minishell.h`.
...
- Wiring in `shell_loop`: extracted into a new `process_line()` static
  ...
  `debug_print_tokens` from steps 3-4 with `debug_print_pipeline` (moved to its
  own new temporary file, `src/shell/debug_pipeline.c`, to keep `shell.c`
...
`inc/structs.h`. The temporary `debug_print_pipeline`/`src/shell/debug_pipeline.c`
...
- **The no-fork exception step 7 deferred** (Decision #17): `execute()`
  ...
  a single command (`!shell->pipeline->next`) whose name is one of the seven
...
### Full regression pass before starting Step 9 (2026-07-31)

Ran before any step-9 code was written, per the subject's "bonus only
evaluated if the mandatory part works perfectly" gate — re-ran the actual
test batches from steps 3-8 (not new ones), to confirm nothing regressed
across the whole mandatory part:

- Clean `make fclean && make`: no errors, no warnings.
- `norminette src inc`: all files `OK`, only the expected
  `GLOBAL_VAR_DETECTED` notice on `g_signal`.
- Tokenizer/validation/parser (steps 3-5): mixed-quote word, all four
  syntax-error cases from step 4, a `<` `|` pipeline, a redir-only stage,
  interleaved redirections, a heredoc — all unchanged from their
  originally-verified behavior.
- Expansion (step 6): `$HOME`, quoted vs. unquoted, unset variable bare
  vs. quoted, greedy variable-name matching, the `pre$TESTVAR`/
  `"pre"$TESTVAR` seam-gluing case — all still correct.
- Execution + builtins (steps 7-8): pipes, all three non-heredoc
  redirections, a heredoc, PATH search (127), every builtin (`echo -n`/
  `-nnn`, `cd` with `PWD`/`OLDPWD` tracking, `export`/`unset` including
  invalid-identifier and word-splitting-exempt-assignment cases, `env`,
  all three `exit` argument cases), and subshell isolation for a builtin
  inside a pipe (`export X=... | true`, `cd ... | true` — confirmed zero
  effect on the parent shell) — all still correct.
- Real pty signal test (step 7): Ctrl-C still correctly kills a running
  foreground child, shell survives, `$?` reads back `130`, shell stays
  responsive afterward.
- `valgrind --leak-check=full` over one large combined batch exercising
  all of the above at once: 0 definitely/indirectly/possibly lost bytes.

No regressions found. The mandatory part (steps 1-8) is confirmed
working as documented, going into step 9.
