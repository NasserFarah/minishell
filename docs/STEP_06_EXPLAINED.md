...
```
One line added (`if (shell->pipeline) expand(shell);`) between parsing and the
still-temporary `debug_print_pipeline`. `expand` mutates `cmd->args`/
`redir->target` on the existing `shell->pipeline` in place — nothing new to
...
The guard against `shell->pipeline` being `NULL` covers step 5's own edge case
(a `malloc` failure inside `parse()`) — expansion has nothing to do if
there's no AST to expand.

`debug_print_pipeline` itself needed no changes: it already prints whatever's
...
`expand_fragments` / `join_word`) -> `debug_print_pipeline` -> `free_cmd` ->
