...
```
Declares (doesn't define) the one global the subject explicitly permits. `extern` means "this variable exists somewhere else (in `signals.c`), just let every `.c` file that includes this header link against it." Three qualifiers matter together here:
- `sig_atomic_t` — a type guaranteed by the C standard to be read/written as a single atomic operation, so a signal handler writing to it can't race with the main program reading it mid-write.
- `volatile` — tells the compiler not to cache this value in a register or optimize away "redundant" reads, because it can change asynchronously between any two instructions (from inside a signal handler), not just through normal control flow.
- `sig_atomic_t` alone doesn't guarantee visibility across optimization — `volatile` is what forces the compiler to re-read memory every time.
...
- `g_signal = sig` — records which signal fired (`SIGINT`'s numeric value) into the atomic global, for the parser/executor to inspect later once heredocs and exit-status reporting need to know "was the last thing interrupted by Ctrl-C".
