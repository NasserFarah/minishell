...
the project's existing convention. `debug_print_ast`'s declaration —
present since step 5 — is now gone entirely, along with the file it
named (`src/shell/debug_ast.c`, deleted this step): the scaffolding every
...
`debug_print_ast(shell->ast)`. `src/shell/debug_ast.c` — the temporary
