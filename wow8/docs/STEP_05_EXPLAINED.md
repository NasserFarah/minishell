...
`src/shell/shell.c` + `src/shell/debug_ast.c` (wiring + temporary
visualization).
...
```
One field added: `ast`, the head of the pipeline `parse()` builds for the
...
void	debug_print_ast(t_cmd *ast);
```
...
headers yet). `debug_print_ast` is explicitly commented as temporary in
...
## 6. `src/shell/shell.c` and `src/shell/debug_ast.c` — wiring it in
...
AST or freed directly). `debug_print_ast` is the direct replacement for
...
`debug_print_ast` (in the new `src/shell/debug_ast.c`, not `shell.c`
...
`debug_print_tokens`, just now aware of the tree-shaped (well, list-shaped)
...
`debug_print_ast` (-> `print_word` / `print_redirs`); on any parser failure
(allocation only — grammar is already guaranteed by step 4),
`shell->ast` stays `NULL` -> `free_cmd` -> loop back to `readline`.
